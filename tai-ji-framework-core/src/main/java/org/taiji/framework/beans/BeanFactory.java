...
//        }
    ...
    private String getBeanName(Class<?> beanClass){
        ...
        if(StringVerify.isBlank(beanName)){
            // 获取接口类
            Class<?>[] interfacesClass = beanClass.getInterfaces();
            // 如果类存在接口则获取第一个接口名称为bean名称
            // TODO 暂未实现多继承的解决方案
            if(interfacesClass.length > 0){
                return StringDenerate.initialLowercase(interfacesClass[0].getSimpleName());
            }
            return StringDenerate.initialLowercase(beanClass.getSimpleName());
        }else {
            return beanName;
        }
    }
